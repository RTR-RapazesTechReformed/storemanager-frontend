...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`${API_CONFIG.BASE_URL}/stores/${deleteStoreId}`, {
            method: 'DELETE',
...
async function loadStores() {
    try {
        ...
        listContainer.innerHTML = '<div class="loading-state"><div class="loading-spinner"></div>Carregando lojas...</div>';
        ...
        const listContainer = document.getElementById('stores-list');
        listContainer.innerHTML = '<div class="loading">Erro ao carregar lojas</div>';
    }
}
...
function renderStores() {
    ...
    container.innerHTML = stores.map(store => `
        <div class="movement-item">
            ...
            ${store.createdAt ? `<p><strong>Cadastrado em:</strong> ${new Date(store.createdAt).toLocaleDateString('pt-BR')}</p>` : ''}
            <div style="display: flex; gap: var(--spacing-sm); margin-top: var(--spacing-md);">
                <button class="btn btn-primary" onclick="openEditModal('${store.id}')">Editar</button>
                <button class="btn btn-danger" onclick="openDeleteModal('${store.id}', '${store.name.replace(/'/g, "\\'")}')">Excluir</button>
            </div>
        </div>
    `).join('');
}
...
async function handleEditSubmit(event) {
    ...
    try {
        const response = await fetch(`${API_CONFIG.BASE_URL}/stores/${editingStoreId}`, {
            method: 'PUT',
            headers: API_CONFIG.HEADERS,
            body: JSON.stringify(storeData)
        });
