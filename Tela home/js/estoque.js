...
function updateStoreSelect() {
    ...
    if (filterSelect && stores.length > 0) {
        filterSelect.innerHTML = '<option value="">Todas as Lojas</option>' +
            stores.map(store => 
    ...
    filteredProducts = products.filter(product => {
        ...
        if (typeFilter) {
            if (product.card && typeFilter === 'CARD') {
                matchType = true;
            } else if (product.other_product && product.other_product.type === typeFilter) {
                matchType = true;
            } else {
                matchType = false;
            }
        ...
        const matchCondition = !conditionFilter || product.condition === conditionFilter;
        ...
        const matchSearch = !searchFilter || 
            ...
            (product.store_name && product.store_name.toLowerCase().includes(searchFilter)) ||
            (product.card && product.card.title && product.card.title.toLowerCase().includes(searchFilter)) ||
            (product.card && product.card.code && product.card.code.toLowerCase().includes(searchFilter));
            ...
            ${filteredProducts.map(product => {
                ...
                if (isCard) {
                    ...
                    productDetails = `
                        ...
                        <p><strong>Tipo:</strong> ${card.pokemon_type || 'N/A'}</p>
                        <p><strong>Nacionalidade:</strong> ${card.nationality || 'N/A'}</p>
                    `;
                    ...
                    productDetails = `
                        <p><strong>Tipo:</strong> ${getProductTypeLabel(other.type)}</p>
                        <p><strong>Nacionalidade:</strong> ${other.nationality || 'N/A'}</p>
                        ${other.package_contents ? `<p><strong>Conteúdo:</strong> ${other.package_contents}</p>` : ''}
                        ${other.extra_info ? `<p><strong>Info:</strong> ${other.extra_info}</p>` : ''}
                    `;
                }
                ...
                return `
                    <div class="product-card ${cardClass}">
                        ...
                        <div class="product-body">
                            ${productDetails}
                            <p class="product-price"><strong>Preço:</strong> R$ ${product.price.toFixed(2)}</p>
                            <p><strong>Condição:</strong> ${getConditionLabel(product.condition)}</p>
                            <p><strong>Loja:</strong> ${product.store_name || 'N/A'}</p>
...
async function confirmDeleteProduct() {
    ...
    try {
        const response = await fetch(`${API_CONFIG.BASE_URL}/products/${deleteProductId}`, {
            method: 'DELETE',
            headers: getHeaders()
        });
...
function editProduct(productId) {
    ...
    if (product.card) {
        ...
        document.getElementById('edit-card-rarity').value = product.card.rarity || '';
        document.getElementById('edit-card-type').value = product.card.pokemon_type || '';
        document.getElementById('edit-card-nationality').value = product.card.nationality || '';
    } else if (product.other_product) {
        otherFields.style.display = 'block';
        document.getElementById('edit-other-nationality').value = product.other_product.nationality || '';
        document.getElementById('edit-other-package').value = product.other_product.package_contents || '';
        document.getElementById('edit-other-info').value = product.other_product.extra_info || '';
    }
...
async function handleEditProductSubmit(event) {
    ...
    try {
        ...
        if (!productResponse.ok) {
            ...
            throw new Error(errorData.message || `Erro HTTP: ${productResponse.status}`);
        }
        
        // Atualizar campos específicos se for carta
        if (editingProduct.card) {
            const cardData = {
                title: document.getElementById('edit-card-title').value.trim() || editingProduct.card.title || '',
                season: editingProduct.card.season || '',
                code: document.getElementById('edit-card-code').value.trim() || editingProduct.card.code || '',
                rarity: document.getElementById('edit-card-rarity').value.trim() || editingProduct.card.rarity || '',
                pokemon_type: document.getElementById('edit-card-type').value.trim() || editingProduct.card.pokemon_type || '',
                nationality: document.getElementById('edit-card-nationality').value.trim() || editingProduct.card.nationality || '',
                collection_id: editingProduct.card.collection_id || ''
            };
            
            const cardResponse = await fetch(`${API_CONFIG.BASE_URL}/cards/${editingProduct.card.id}`, {
                method: 'PUT',
                headers: getHeaders(),
            ...
            if (!cardResponse.ok) {
                ...
                throw new Error(`Erro ao atualizar carta: ${errorData.message || cardResponse.status}`);
            }
        } 
        // Atualizar campos específicos se for outro produto
            ...
            const otherProductData = {
                ...
                nationality: document.getElementById('edit-other-nationality').value.trim() || '',
                package_contents: document.getElementById('edit-other-package').value.trim() || '',
                extra_info: document.getElementById('edit-other-info').value.trim() || ''
            };
            
            const otherResponse = await fetch(`${API_CONFIG.BASE_URL}/other-products/${editingProduct.other_product.id}`, {
                method: 'PUT',
                headers: getHeaders(),
            ...
            if (!otherResponse.ok) {
                ...
                throw new Error(`Erro ao atualizar produto: ${errorData.message || otherResponse.status}`);
            }
        }
