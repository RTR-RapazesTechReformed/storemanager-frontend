...
function initializeEventListeners() {
    ...
    if (typeSelect) typeSelect.addEventListener('change', handleProductTypeChange);
    if (targetStoreSelect) targetStoreSelect.addEventListener('change', handleTargetStoreChange);
...
function handleProductTypeChange(event) {
    ...
    otherFields.querySelectorAll('input, textarea').forEach(input => input.value = '');
...
function handleTargetStoreChange(event) {
    ...
    const existingProductIdGroup = document.getElementById('existing-product-id-group');
...
async function handleProductSubmit(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showAlert(`Erro ao cadastrar produto: ${responseData.message || response.statusText}`, 'error');
        }
    } catch (error) {
...
function clearForm() {
    ...
    if (form) {
        ...
        if (cardFields) cardFields.querySelectorAll('input').forEach(input => input.value = '');
        if (otherFields) otherFields.querySelectorAll('input, textarea').forEach(input => input.value = '');
    }
}
